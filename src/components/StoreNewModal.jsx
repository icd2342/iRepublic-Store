...
const ProductCard = ({ model, image, price, storage, colors, colorData }) => {
  ...
  const handleBuy = () => {
    const message = `Здравствуйте! Интересует новый ${model} ${selectedStorage} ${selectedColor.name} за ${price} ${settings.currency}`;
...
const StoreNewModal = ({ isOpen, onClose }) => {
  ...
  const fetchProducts = async () => {
    const response = await axios.get('https://admin-dashboard-qff2.vercel.app/api/product?category=new');
    ...
    <Modal isOpen={isOpen} onClose={onClose}>
      <div className="w-full">
        ...
        <div className="text-center mb-8">
          <h1 className="text-4xl font-bold mb-2">Новые устройства</h1>
          <p className="text-gray-400">Купить новые устройства в iRepublic</p>
        </div>
